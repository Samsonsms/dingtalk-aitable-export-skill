...
- 支持超时续跑，并精确清理本次创建的临时展示视图

> 当前版本：**1.0.0**。支持 Windows 10/11 PowerShell、Python 3.12+、Node.js/npm、官方 DWS CLI 和 `curl.exe`。

> **不会删除表格内容：**“视图”只是同一份数据的展示配置。技能只会清理自己为导出创建的临时视图，不会删除或修改数据表、字段、记录以及用户已有的视图。
...
这里的“视图”不是数据表或表格内容，而是字段显示、筛选和排序等展示配置。临时视图只在选表后创建，技能只删除状态文件中记录的本次 `viewId`，不会按名称批量删除，也不会操作任何已有视图。

整个流程不会调用删除数据表、字段或记录的命令。清理失败时，最坏结果是钉钉中残留一个临时视图，业务数据仍保持不变。
...
只有返回 `status=complete` 才代表导出、校验和临时展示视图清理全部完成。这里的清理不涉及表格数据。超时返回 `status=pending` 时，应使用原状态文件继续轮询同一个任务，不能重新创建视图或重复提交导出。
